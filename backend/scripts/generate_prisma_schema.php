...
function mapType($mysqlType, $nullable, &$dbAttr) {
    $dbAttr = '';
...
foreach ($data as $tableName => $tableInfo) {
    ...
    foreach ($cols as $c) {
        ...
        $nullable = ($c['null'] === 'YES');
        $dbAttr = '';
        $type = mapType($c['type'], $nullable, $dbAttr);
        ...
        if ($c['default'] !== null && !$isAutoInc && !$isPk) {
            if ($c['default'] === 'CURRENT_TIMESTAMP' || strpos($c['default'], 'current_timestamp') !== false) {
...
echo "Generated schema.prisma successfully at frontend/prisma/schema.prisma (" . strlen($output) . " bytes)\n";
