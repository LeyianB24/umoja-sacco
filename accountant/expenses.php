...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_expense') {
    ...
    if ($amount <= 0) {
        ...
        try {
            ...
            // 1. Record in Central Transactions Table
            $stmt = $conn->prepare("INSERT INTO transactions (member_id, transaction_type, amount, reference_no, notes, related_id, related_table, created_at, recorded_by) VALUES (NULL, 'expense', ?, ?, ?, ?, 'investments', ?, ?)");
            $timestamp = $date . ' ' . date('H:i:s');
            $stmt->bind_param("dssisi", $amount, $ref_no, $notes, $inv_id, $timestamp, $_SESSION['admin_id']);
            $stmt->execute();
            $stmt->close();

            // 2. Record in Specific Investment Expense Table for Profit/Loss tracking
            if ($inv_id) {
                // Check if it's a vehicle fleet item
                $check = $conn->query("SELECT category FROM investments WHERE investment_id = $inv_id")->fetch_assoc();
                if($check && $check['category'] == 'vehicle_fleet') {
                    $ins = $conn->prepare("INSERT INTO vehicle_expenses (vehicle_id, amount, expense_date, category, description, recorded_by) VALUES (?, ?, ?, ?, ?, ?)");
                    $ins->bind_param("idsssi", $inv_id, $amount, $timestamp, $cat, $desc, $_SESSION['admin_id']);
                } else {
                    $ins = $conn->prepare("INSERT INTO investment_expenses (investment_id, amount, expense_date, description, recorded_by) VALUES (?, ?, ?, ?, ?)");
                    $ins->bind_param("idssi", $inv_id, $amount, $timestamp, $desc, $_SESSION['admin_id']);
                }
                $ins->execute();
                $ins->close();
            }
