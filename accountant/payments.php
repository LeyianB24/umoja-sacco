...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'record_txn') {
    ...
    if ($amount <= 0) {
        ...
        try {
            // A. Insert into Transactions (Ledger - Always)
            $stmt = $conn->prepare("INSERT INTO transactions (member_id, transaction_type, amount, reference_no, notes, created_at) VALUES (?, ?, ?, ?, ?, ?)");
            $timestamp = $date . ' ' . date('H:i:s'); 
            $stmt->bind_param("isdsss", $member_id, $type, $amount, $ref_no, $notes, $timestamp);
            $stmt->execute();
            $txn_id = $conn->insert_id;
            $stmt->close();

            // B. Specific Type Handlers
            if ($member_id) {
                // 1. DEPOSITS (Savings)
                if ($type === 'deposit') {
                    // Update Balance
                    $conn->query("UPDATE members SET account_balance = account_balance + $amount WHERE member_id = $member_id");
                    
                    // Record in Contributions
                    $stmt = $conn->prepare("INSERT INTO contributions (member_id, contribution_type, amount, payment_method, reference_no, status, created_at) VALUES (?, 'savings', ?, 'cash', ?, 'active', ?)");
                    $stmt->bind_param("idss", $member_id, $amount, $ref_no, $timestamp);
                    $stmt->execute();
                    $stmt->close();
                } 
                // 2. WITHDRAWALS
                elseif ($type === 'withdrawal') {
                    ...
                    $res = $conn->query("SELECT account_balance FROM members WHERE member_id = $member_id");
                    $bal = $res->fetch_assoc()['account_balance'];
                    if ($bal < $amount) {
                        throw new Exception("Insufficient member balance for withdrawal.");
                    }
                    $conn->query("UPDATE members SET account_balance = account_balance - $amount WHERE member_id = $member_id");
                } 
                // 3. LOAN REPAYMENT
                elseif ($type === 'loan_repayment') {
                    // Find active loan (priority: disbursed)
                    $l_res = $conn->query("SELECT loan_id, current_balance FROM loans WHERE member_id = $member_id AND status = 'disbursed' LIMIT 1");
                    if ($l_res->num_rows > 0) {
                        $loan = $l_res->fetch_assoc();
                        $lid = $loan['loan_id'];
                        $cur_bal = $loan['current_balance'];
                        
                        // Remaining logic handled by trigger, but we need to pass PRE-payment balance to history if we want
                        // Using Trigger logic: Insert payment -> Trigger updates Loan.
                        
                        // Just insert into loan_repayments
                        // Calculate expected new balance for record (trigger handles actual update)
                        $rem_bal = max(0, $cur_bal - $amount);
                        
                        $stmt = $conn->prepare("INSERT INTO loan_repayments (loan_id, amount_paid, payment_date, payment_method, reference_no, remaining_balance, status) VALUES (?, ?, ?, 'cash', ?, ?, 'Completed')");
                        $stmt->bind_param("idssd", $lid, $amount, $timestamp, $ref_no, $rem_bal);
                        $stmt->execute();
                        $stmt->close();
                    } else {
                        // Warn but allow? Or Block? Let's Block to ensure data integrity.
                        // Actually, if paying off a written_off loan or old loan?
                        // Let's assume strict: Must have active loan.
                        throw new Exception("No active/disbursed loan found for this member.");
                    }
                } 
                // 4. SHARE CAPITAL
                elseif ($type === 'share_capital') {
                    // Update Shares Table
                    // Assuming price = 100 or fetched from config. defaulting 100.
                    $unit_price = 100; 
                    $units = $amount / $unit_price;
                    
                    // Shares table insert
                    $stmt = $conn->prepare("INSERT INTO shares (member_id, share_units, unit_price, purchase_date) VALUES (?, ?, ?, ?)");
                    $stmt->bind_param("idss", $member_id, $units, $unit_price, $timestamp);
                    $stmt->execute();
                    $stmt->close();

                    // Contributions table (for record keeping/analytics)
                    $stmt = $conn->prepare("INSERT INTO contributions (member_id, contribution_type, amount, payment_method, reference_no, status, created_at) VALUES (?, 'shares', ?, 'cash', ?, 'active', ?)");
                    $stmt->bind_param("idss", $member_id, $amount, $ref_no, $timestamp);
                    $stmt->execute();
                    $stmt->close();
                }
