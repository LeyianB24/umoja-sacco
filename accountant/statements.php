...
if ($member_id) {
    ...
    $sql_open = "SELECT 
        SUM(CASE WHEN transaction_type IN ('deposit', 'repayment', 'income', 'share_capital', 'welfare_sinking') THEN amount ELSE 0 END) -
        SUM(CASE WHEN transaction_type IN ('withdrawal', 'loan_disbursement', 'expense', 'loan') THEN amount ELSE 0 END) as balance
        FROM transactions 
                        ...
                        <tbody>
                            ...
                            <?php else: 
                                foreach ($transactions as $t): 
                                    ...
                                    $is_credit = in_array($type, ['deposit', 'repayment', 'income', 'share_capital', 'welfare_sinking']);
