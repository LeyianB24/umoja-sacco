...
Auth::requireAdmin();
require_permission('revenue.php'); // ← fixed: matches DB key (was 'record_income')
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_revenue'])) {
    ...
    // Safe split — guard against missing underscore in posted value
    $unified_id    = $_POST['unified_asset_id'] ?? 'other_0';
    $parts         = explode('_', $unified_id, 2);
    $source_prefix = $parts[0] ?? 'other';
    $related_id    = (int)($parts[1] ?? 0);

    if ($related_id <= 0 && $source_prefix !== 'other') {
        flash_set('Please select a valid revenue source.', 'error');
    } elseif ($amount <= 0) {
        flash_set('Amount must be greater than zero.', 'error');
    } else {
        $config = [
            'type'             => 'income',
        ...
        if (TransactionHelper::record($config)) {
            ...
            flash_set('Failed to record revenue. Please try again.', 'error');
        }
    }
}
...
$target_row        = $conn->query("SELECT SUM(target_amount) AS total_target FROM investments WHERE status = 'active'")->fetch_assoc();
$total_targets     = (float)($target_row['total_target'] ?? 0);
$global_target_pct = $total_targets > 0 ? min(100, ($total_period_rev / $total_targets) * 100) : 0;

// ── 5. Per-Asset Revenue + Viability ─────────────────────────────────────────
$stmt_assets = $conn->prepare("
    SELECT
        i.investment_id  AS id,
        i.title,
        i.category,
        i.target_amount,
        i.target_period,
        i.viability_status,
        'investments'    AS asset_table,
        SUM(CASE WHEN t.transaction_date BETWEEN ? AND ? THEN t.amount ELSE 0 END) AS period_revenue,
        COALESCE(SUM(t.amount), 0)   AS total_revenue,
        COUNT(t.transaction_id)      AS transaction_count
    FROM investments i
    LEFT JOIN transactions t
           ON t.related_table = 'investments'
          AND t.related_id    = i.investment_id
          AND t.transaction_type IN ($rev_types)
...
foreach ($all_assets_raw as $asset) {
    ...
    $asset['target_achievement'] = $perf['target_achievement_pct'] ?? 0;
    $asset['is_profitable']      = $perf['is_profitable']          ?? false;
    $asset['net_profit']         = $perf['net_profit']             ?? 0;
    $asset['viability_status']   = $perf['viability_status']       ?? $asset['viability_status'];
...
// ── 6. Investment select lists ────────────────────────────────────────────────
$inv_js_data             = [];
$investments_select_list = [];
$inv_res = $conn->query("SELECT investment_id, title, target_amount, target_period FROM investments WHERE status = 'active'");
...
if (in_array($action, ['export_pdf', 'export_excel', 'print_report'], true)) {
    ...
    foreach ($revenue_data as $row) {
        $export_rows[] = [
            ...
            'Source'  => $row['source_name']  ?? 'General Fund',
            'Method'  => strtoupper($row['payment_method'] ?? 'N/A'),
            'Amount'  => number_format((float)$row['amount'], 2),
            'Details' => $row['notes'] ?? '',
        ];
    }

    $file_title = 'Revenue_Ledger_' . date('Ymd_His');
    $headers    = ['Date', 'Ref', 'Source', 'Method', 'Amount', 'Details'];

    if ($action === 'export_pdf') {
        require_once __DIR__ . '/../../inc/ExportHelper.php';
        ExportHelper::pdf('Revenue Ledger', $headers, $export_rows, $file_title . '.pdf');

    } elseif ($action === 'export_excel') {
        require_once __DIR__ . '/../../inc/ExportHelper.php';
        ExportHelper::csv($file_title . '.csv', $headers, $export_rows);

    } else {
        require_once __DIR__ . '/../../core/exports/UniversalExportEngine.php';
        UniversalExportEngine::handle('print', $export_rows, [
...
.page-header {
    ...
    box-shadow: var(--shadow-lg); animation: fadeUp .35s ease both;
}
.page-header::before {
...
.kpi-card {
    ...
    height: 100%; transition: var(--transition); animation: fadeUp .4s ease both;
}
...
.filter-select {
    ...
    transition: var(--transition); cursor: pointer; appearance: auto;
}
...
.date-row input {
    ...
    background: var(--surface-2); padding: 0 10px; transition: var(--transition);
}
...
.search-wrap input {
    width: 100%; padding: 8px 12px 8px 32px; font-size: .82rem; font-weight: 500;
...
.rev-ref    { font-size: .73rem; color: var(--muted); font-family: monospace; margin-top: 2px; }
.rev-source { font-size: .85rem; font-weight: 700; color: var(--ink); }
.rev-notes  { font-size: .74rem; color: var(--muted); margin-top: 2px; }
.method-badge { display: inline-flex; align-items: center; font-size: .7rem; font-weight: 700; letter-spacing: .3px; border: 1.5px solid var(--border); border-radius: 8px; padding: 4px 12px; background: var(--surface-2); color: var(--ink); }
...
<div class="main-content-wrapper">
    ...
    <div class="container-fluid px-4 py-4 page-canvas">
        ...
        <div class="page-header mb-4">
            <div class="hero-inner">
                ...
                <div class="hero-actions">
                    <button class="btn btn-lime rounded-pill px-4 fw-bold shadow-sm"
                            data-bs-toggle="modal" data-bs-target="#recordRevenueModal">
                        <i class="bi bi-plus-circle-fill me-2"></i>Record New Inflow
                    </button>
                    <div class="dropdown">
                        <button class="btn btn-outline-hero rounded-pill px-4 fw-bold dropdown-toggle"
                                data-bs-toggle="dropdown">
                            <i class="bi bi-download me-2"></i>Export
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0" style="border-radius:12px;overflow:hidden">
                            <li><a class="dropdown-item py-2" href="?<?= http_build_query(array_merge($_GET, ['action' => 'export_pdf'])) ?>">
                                <i class="bi bi-file-pdf me-2 text-danger"></i>Revenue Ledger (PDF)</a></li>
                            <li><a class="dropdown-item py-2" href="?<?= http_build_query(array_merge($_GET, ['action' => 'export_excel'])) ?>">
                                <i class="bi bi-file-spreadsheet me-2 text-success"></i>Export Spreadsheet</a></li>
                            <li><hr class="dropdown-divider my-1"></li>
                            <li><a class="dropdown-item py-2" href="?<?= http_build_query(array_merge($_GET, ['action' => 'print_report'])) ?>" target="_blank">
                                <i class="bi bi-printer me-2 text-muted"></i>Print Report</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="mb-4">
            <div class="section-heading">
                ...
                <a href="investments.php" class="btn btn-sm rounded-pill px-3"
                   style="border:1.5px solid var(--border);color:var(--forest);font-size:.78rem;font-weight:700;transition:var(--transition)">
            ...
            <?php foreach ($top_investments as $idx => $inv):
                ...
                $card_class = match($v) { 'viable' => 'viable', 'underperforming' => 'marginal', 'loss_making' => 'risk', default => 'new-asset' };
                $pill_class = match($v) { 'viable' => 'viable', 'underperforming' => 'marginal', 'loss_making' => 'risk', default => 'new-pill' };
                $pill_text  = match($v) { 'viable' => 'Viable', 'underperforming' => 'Marginal', 'loss_making' => 'At Risk', default => 'New' };
            ...
            <div class="col-md-4" style="animation-delay:<?= $idx * .06 ?>s">
                <div class="portfolio-card <?= $card_class ?>">
                    ...
                    <div class="progress-track">
                        <div class="progress-fill-g <?= $pf_class ?>" style="width:<?= min(100, $ach) ?>%"></div>
                    </div>
                    <div class="asset-footer">
                        <span class="txn-count"><i class="bi bi-clock-history"></i><?= (int)$inv['transaction_count'] ?> inputs</span>
                        <span class="profit-val <?= $inv['net_profit'] >= 0 ? 'pos' : 'neg' ?>">
                            Profit: KES <?= number_format((float)$inv['net_profit']) ?>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="kpi-card" style="animation-delay:.08s">
                    ...
                    <div class="kpi-progress-track">
                        <div class="kpi-progress-fill" style="width:<?= $global_target_pct ?>%"></div>
                    </div>
                                ...
                                <select name="asset_id" class="filter-select" onchange="this.form.submit()">
                                    ...
                                    <?php foreach ($investments_select_list as $inv): ?>
                                        <option value="<?= $inv['investment_id'] ?>"
                                            <?= $filter_asset_id == $inv['investment_id'] ? 'selected' : '' ?>>
                                            <?= htmlspecialchars((string)($inv['title'] ?? 'Asset ' . $inv['investment_id'])) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
        ...
        <div class="modal fade" id="recordRevenueModal" tabindex="-1" aria-labelledby="recordRevenueLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header-forest">
                        <div style="display:flex;align-items:center;justify-content:space-between;width:100%">
                            <h5 id="recordRevenueLabel"><i class="bi bi-shield-check me-2"></i>Record Received Inflow</h5>
                    ...
                    <form method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="unified_asset_id" id="unified_asset_id"
                            ...
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" name="revenue_date" class="form-control form-control-lg"
                            ...
                            <div class="mb-1">
                                ...
                                <textarea name="description" class="form-control"
                                          placeholder="e.g. Daily collection, Dividend check…"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer justify-content-end gap-2">
                            <button type="button" class="btn btn-light rounded-pill fw-bold px-4" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-lime rounded-pill fw-bold px-5 text-dark">
                                <i class="bi bi-check2-circle me-2"></i>Confirm & Post Ledger
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <!-- ═══ SCRIPTS — placed before footer() so they're inside </body> ══ -->
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
    // Investment data for target info box
    const invData = <?= json_encode($inv_js_data, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    ...
    function updateTargetInfo() {
        ...
        if (val.startsWith('inv_')) {
            ...
            if (data && parseFloat(data.target_amount) > 0) {
                txt.innerHTML = `Target: <strong>KES ${new Intl.NumberFormat('en-KE').format(data.target_amount)}</strong> (${data.target_period})`;
