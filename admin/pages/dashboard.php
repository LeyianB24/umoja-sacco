...
/**
 * admin/dashboard.php
 * Admin Console – V28 Premium Hope UI
 * Design: High-Fidelity Glassmorphism
 */
...
// --- METRICS ---
$open_tickets = $conn->query("SELECT COUNT(*) AS c FROM support_tickets WHERE status!='Closed'")->fetch_assoc()['c'] ?? 0;
...
// Recent Tickets
$tickets = $conn->query("SELECT s.*, COALESCE(m.full_name,'Guest') AS sender FROM support_tickets s LEFT JOIN members m ON s.member_id=m.member_id WHERE s.status!='Closed' ORDER BY s.created_at DESC LIMIT 5")->fetch_all(MYSQLI_ASSOC);
