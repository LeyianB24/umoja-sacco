...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_employee') {
        ...
        if (empty($name) || empty($nid)) {
            $msg = "Name and ID are required."; $msg_type = "danger";
        } else {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_employee') {
        ...
        if ($stmt->execute()) {
            ...
            $msg = "Employee record updated."; $msg_type = "success";
        }
    }
}
...
<div class="d-flex">
        ...
        <div class="flex-fill main-content-wrapper" style="margin-left: 280px; transition: margin-left 0.3s ease;">
            ...
            <?php if($msg): ?>
                <div class="alert alert-<?= $msg_type ?> border-0 shadow-sm alert-dismissible fade show mb-4 rounded-3 d-flex align-items-center" role="alert">
                    <i class="bi bi-<?= $msg_type=='success'?'check-circle':'exclamation-circle' ?>-fill fs-4 me-3"></i>
                    <div><?= $msg ?></div>
                    ...
                    <div class="hd-glass p-4 h-100 d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <h2 class="fw-bold mb-0 text-success">
                                <small class="fs-6 text-muted fw-normal">KES</small> <?= number_format($monthly_payroll) ?>
                            </h2>
                        </div>
