...
if ($table_check->num_rows == 0) {
    ...
    if ($db->query($create_sql)) {
        flash_set("System setup: 'expenses' table created successfully.", 'info');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_expense'])) {
    ...
    if ($amount <= 0 || empty($desc)) {
        ...
        if ($stmt->execute()) {
            ...
            flash_set("Error recording expense: " . $db->error, 'error');
        }
    }
}
...
<head>
    ...
    <style>
        ...
        body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #f4f7fe; transition: background 0.3s; }
