...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($member_id > 0 && $amount > 0 && !empty($reason)) {
        ...
        try {
            ...
            // B. Credit Member Wallet (Internal Transfer: Sacco -> Member)
            // This allows the member to "Withdraw it too" as requested.
            $stmt = $conn->prepare("UPDATE members SET account_balance = account_balance + ? WHERE member_id = ?");
            $stmt->bind_param("di", $amount, $member_id);
            $stmt->execute();
            $stmt->close();
            
            // C. Record Transaction (Expense for Sacco)
            // We treat this as an expense immediately in the books, even if it sits in their wallet.
            $notes = "Welfare Grant: " . $reason;
            $ref = "WS-" . str_pad($support_id, 6, '0', STR_PAD_LEFT);
            
            $stmt = $conn->prepare("INSERT INTO transactions (member_id, transaction_type, amount, related_id, reference_no, notes, created_at, payment_channel) VALUES (?, 'expense', ?, ?, ?, ?, NOW(), 'system')");
            $stmt->bind_param("idiss", $member_id, $amount, $support_id, $ref, $notes);
            $stmt->execute();
            $stmt->close();
            
            // D. Notify Member
            // (Assuming notification logic)
            $msg = "You have been granted Welfare Support of KES " . number_format($amount) . ". Reason: $reason. Funds added to your wallet.";
