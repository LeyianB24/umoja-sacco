    ...
    if ($loan_id > 0) {
        ...
        if ($new_status) {
            ...
            try {
                ...
                $_SESSION['flash_msg'] = "Loan #$loan_id has been " . strtoupper($new_status);
                $_SESSION['flash_type'] = $new_status === 'approved' ? 'success' : 'warning';

            } catch (Exception $e) {
                $db->rollback();
                $_SESSION['flash_msg'] = "Error processing loan: " . $e->getMessage();
...
function ksh($val) { return number_format((float)$val, 2); }
?>
<!DOCTYPE html>
...
<div class="d-flex">
        ...
        <div class="flex-fill main-content-wrapper" style="margin-left: 280px; transition: margin-left 0.3s ease;">
            ...
            <?php if($msg): ?>
                <div class="alert alert-<?= $msg_type ?> alert-dismissible fade show border-0 shadow-sm" role="alert">
