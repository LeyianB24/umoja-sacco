...
// Enforce Clerk or Admin/Superadmin
if (!isset($_SESSION['role']) || !in_array($_SESSION['role'], ['clerk', 'admin', 'superadmin'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // 2. Insert Member
            $ins = $conn->prepare("INSERT INTO members (member_reg_no, full_name, national_id, phone, email, password, join_date, status, reg_fee_paid) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?)");
            $ins->bind_param("sssssssi", $reg_no, $full_name, $national_id, $phone, $email, $hashed, $status, $paid);
            ...
            if ($paid) {
                ...
                $stmt = $conn->prepare("INSERT INTO contributions (member_id, contribution_type, amount, payment_method, reference_no, status, created_at, created_by_admin) VALUES (?, 'registration', 1000.00, ?, ?, 'active', NOW(), ?)");
                $stmt->bind_param("issi", $member_id, $pay_method, $ref, $_SESSION['admin_id']);
                $stmt->execute();
                
                // Record in transactions ledger
                $stmt = $conn->prepare("INSERT INTO transactions (member_id, transaction_type, amount, payment_channel, notes, reference_no, created_at, recorded_by) VALUES (?, 'deposit', 1000.00, ?, 'Registration Fee Payment', ?, NOW(), ?)");
                $stmt->bind_param("isssi", $member_id, $pay_method, $ref, $_SESSION['admin_id']);
                $stmt->execute();
            }
    ...
    <div class="flex-fill" style="margin-left: 280px; padding: 20px;">
        ...
        <div class="container-fluid py-4">
            <div class="row justify-content-center">
                <div class="col-lg-7">
                    <div class="mb-4">
                        <h2 class="fw-bold">Member Onboarding</h2>
                        <p class="text-secondary">Register a new member and collect registration fee.</p>
                    </div>
