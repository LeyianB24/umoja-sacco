...
if (isset($_POST['run_seed'])) {
    foreach ($permissions as $p) {
        $stmt = $conn->prepare("INSERT IGNORE INTO permissions (permission_key, display_name, category) VALUES (?, ?, ?)");
    ...
    $initial_mappings = [
        'manager' => ['member_directory', 'loan_reviews', 'welfare_cases', 'investment_mgmt', 'fleet_mgmt', 'employee_mgmt', 'tech_support'],
        'accountant' => ['member_directory', 'revenue_tracking', 'payment_processing', 'expense_mgmt', 'loan_disbursement', 'financial_statements', 'financial_reports', 'tech_support'],
        'admin' => ['member_directory', 'audit_logs', 'system_backups', 'system_settings', 'tech_support'],
        'clerk' => ['member_directory', 'member_registration', 'tech_support']
    ];

    foreach ($initial_mappings as $role_name => $keys) {
        $r_stmt = $conn->prepare("SELECT role_id FROM roles WHERE role_name = ?");
        $r_stmt->bind_param("s", $role_name);
        $r_stmt->execute();
        $role_id = $r_stmt->get_result()->fetch_assoc()['role_id'] ?? 0;
        $r_stmt->close();
        
        if ($role_id > 0) {
            foreach ($keys as $key) {
                $p_stmt = $conn->prepare("SELECT id FROM permissions WHERE permission_key = ?");
