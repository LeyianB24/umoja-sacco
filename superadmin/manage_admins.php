...
$defined_roles = [
    ...
    'clerk'      => ['label' => 'Clerk',      'color' => 'bg-info']
];
...
$admins_count = $res->num_rows;
echo "<!-- DEBUG: Admins Found in PHP: $admins_count -->";
if($admins_count > 0) {
    echo "<div class='alert alert-info py-1 mb-0' style='position:fixed; bottom:10px; right:10px; z-index:9999;'>PHP Admins Found: $admins_count</div>"; 
} else {
    echo "<div class='alert alert-warning py-1 mb-0' style='position:fixed; bottom:10px; right:10px; z-index:9999;'>PHP Admins: NONE FOUND</div>"; 
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        body { font-family: var(--font-main); background-color: var(--bg-primary); color: var(--text-main); }
        .iq-card { background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 20px; overflow: hidden; box-shadow: var(--shadow-sm); }
        .admin-row:hover { background-color: rgba(0,0,0,0.02); }
        [data-bs-theme="dark"] .admin-row:hover { background-color: rgba(255,255,255,0.02); }
    </style>
</head>
