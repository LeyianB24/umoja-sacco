<?php
// core/exports/ExportManager.php
/**
 * @deprecated Use FinancialExportEngine instead.
 * This class is left for backward compatibility but should not be used in new code.
 */
...
class ExportManager {
    ...
    private static function generateExcel($data, $headers, $title, $module, $outputMode) {
        if ($outputMode === 'S') {
            ob_start();
            $excel = new ExcelTemplate($data, $headers, $title, $module);
            $excel->generate(false);
            return ob_get_clean();
        } else {
            $excel = new ExcelTemplate($data, $headers, $title, $module);
            $excel->generate(true);
