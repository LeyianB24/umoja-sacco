...
if ($type === 'savings') {
    ...
    $withdrawal_sources['welfare']['balance'] = max(0, $totalIn - $totalOut);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['withdraw'])) {
    ...
    if ($amount <= 0) {
        ...
        try {
            ...
            // 1. Deduct from appropriate source
            if ($type === 'wallet' || $type === 'loans') {
                $stmt = $conn->prepare("UPDATE members SET account_balance = account_balance - ? WHERE member_id = ?");
                $stmt->bind_param("di", $amount, $member_id);
                $stmt->execute();
                $stmt->close();
            } elseif ($type === 'savings') {
                $stmt = $conn->prepare("INSERT INTO savings (member_id, amount, transaction_type, created_at) 
                                       VALUES (?, ?, 'withdrawal', NOW())");
                $negative_amount = -$amount;
                $stmt->bind_param("id", $member_id, $negative_amount);
                $stmt->execute();
                $stmt->close();
            } elseif ($type === 'shares') {
                // Deduct from shares table (negative record)
                $stmt = $conn->prepare("INSERT INTO shares (member_id, share_units, unit_price, total_value, reference_no, created_at) 
                                       VALUES (?, ?, ?, ?, ?, NOW())");
                $units_to_deduct = $amount / 100; // Assuming 100 per unit
                $negative_amount = -$amount;
                $ref_shares = 'WD-SH-' . strtoupper(substr(md5(uniqid()), 0, 5));
                $unit_price = 100;
                $stmt->bind_param("iddds", $member_id, $units_to_deduct, $unit_price, $negative_amount, $ref_shares);
                $stmt->execute();
                $stmt->close();
            } elseif ($type === 'welfare') {
                // Record special contribution to deduct from welfare
                $stmt = $conn->prepare("INSERT INTO contributions (member_id, amount, contribution_type, payment_method, reference_no, status, created_at) 
                                       VALUES (?, ?, 'welfare', 'system', ?, 'completed', NOW())");
                $negative_amount = -$amount;
                $ref_welfare = 'WD-WF-' . strtoupper(substr(md5(uniqid()), 0, 5));
                $stmt->bind_param("idss", $member_id, $negative_amount, $ref_welfare);
                $stmt->execute();
                $stmt->close();
            }
            ...
            $stmt = $conn->prepare("INSERT INTO transactions (member_id, transaction_type, amount, reference_no, notes, payment_channel, created_at) 
                                    VALUES (?, 'withdrawal', ?, ?, ?, 'paystack', NOW())");
            $notes = "Withdrawal from " . $current_source['title'] . " to $phone";
            $stmt->bind_param("idss", $member_id, $amount, $ref, $notes);
            $stmt->execute();
            $transaction_id = $conn->insert_id;
            $stmt->close();
            
            // 3. Initiate Paystack transfer (with M-Pesa fallback)
            require_once __DIR__ . '/../inc/paystack_lib.php';
        ...
        <div class="row justify-content-center">
            <div class="col-lg-6">
                ...
                <div class="balance-display mb-4">
                    ...
                    <h1 class="display-4 fw-bold mb-0">KES <?= number_format($max_withdrawal, 2) ?></h1>
                </div>
                ...
                <div class="card card-custom p-4">
                    <form method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <small class="text-muted">Maximum: KES <?= number_format($max_withdrawal, 2) ?></small>
                        </div>
                ...
                <div class="text-center mt-4">
                    <a href="<?= BASE_URL ?>/member/transactions.php" class="text-decoration-none">
